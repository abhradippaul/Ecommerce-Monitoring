 ...
 * - The majority (maximum) of requests complete fast around ~5ms - 50ms.
 * - A minority (minimum) of requests simulate traffic spikes/slowness and take near ~1s (e.g. 800ms - 1100ms).
 *
 * Configurable via environment variables:
 * - ENABLE_SLOWNESS / SIMULATE_SLOWNESS / ENABLE_TRAFFIC_SLOWNESS / ENABLE_ORDER_SLOWNESS: 'true' / 'false'
 * - SLOW_REQUEST_RATIO: fraction of requests that hit the slow spike path (default: 0.15 for 15%)
 * - FAST_REQUEST_MIN_MS: minimum latency for normal requests (default: 5)
 * - FAST_REQUEST_MAX_MS: maximum latency for normal requests (default: 50)
...
export const simulateSlowness = async (spanName: string = 'simulateTrafficSlowness') => {
  const isEnabled =
    ...
    process.env.ENABLE_ORDER_SLOWNESS === 'true' ||
    process.env.SIMULATE_ORDER_SLOWNESS === 'true';

  if (!isEnabled) return;

  const slowRequestRatio = parseFloat(process.env.SLOW_REQUEST_RATIO || '0.15');
  const fastMinMs = parseInt(process.env.FAST_REQUEST_MIN_MS || '5', 10);
  const fastMaxMs = parseInt(process.env.FAST_REQUEST_MAX_MS || '50', 10);
